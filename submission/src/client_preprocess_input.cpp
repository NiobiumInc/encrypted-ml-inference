...
// HEIR v2 preprocessing: MNIST normalization (subtract mean, divide std).
#include "utils.h"
#include "params.h"
...
int main(int argc, char* argv[]){
    ...
    // Normalize the inputs (MNIST mean=0.1307, std=0.3081)
    for (auto& sample : dataset) {
        for (int i = 0; i < MNIST_DIM; ++i) {
            sample.image[i] = (sample.image[i] - 0.1307) / 0.3081;
        }
    }
